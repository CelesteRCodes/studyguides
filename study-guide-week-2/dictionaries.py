...
def top_chars(phrase):
    ...
    #return sorted(letter_counts.values())

    #return sorted(sorted(pair) for pair in letter_counts)

    new_list =[]
    letter_counts = {}
    for letter in phrase:
        letter_counts[letter] = letter_counts.get(letter, 0) + 1            
        new_list = sorted(letter_counts.items())            # returns list of keys and values
    return new_list
...
# append list of chars to a dictionary, where the key = char, value = number of times it appears
top_chars("Shake it off, shake it off.")
